 ...
 */
import { logAgentAction } from '../moltbook/ledger.js';
 ...
 */
async function validateTelegram(creds) {
    if (!creds.botToken) {
 ...
 */
async function validateX(creds) {
 ...
 */
async function validateReddit(creds) {
 ...
 */
async function validateDiscord(creds) {
    if (!creds.webhookUrl) {
 ...
 */
async function openCircuitBreaker(platform, result, moltbookDir, storeDir) {
 ...
 */
export function getValidPlatforms(results) {
    const valid = [];
    for (const [platform, result] of results) {
        if (result.valid) {
            valid.push(platform);
