...
import { readEventsByDate, getTopDenyReasons, } from './reader.js';
 ...
 */
export function generateSummary(events, period, periodType) {
    const byStatus = {
    ...
    const successfulPosts = events.filter(e => e.event_type === 'PLATFORM_RESULT' &&
        e.data.success === true).map(e => ({
    ...
    const cbChanges = events.filter(e => e.event_type === 'CIRCUIT_BREAKER_OPENED' ||
        e.event_type === 'CIRCUIT_BREAKER_CLOSED').map(e => ({
        ts: e.ts,
        platform: e.platform || 'unknown',
        action: e.event_type === 'CIRCUIT_BREAKER_OPENED' ? 'opened' : 'closed',
    }));
 ...
 */
export function generateWeeklySummary(year, week) {
    const now = new Date();
 ...
 */
export function backfillSummaries(startDate, endDate) {
