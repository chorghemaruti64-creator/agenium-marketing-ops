...
import { appendEvent, appendEventBatch, logPolicyDecisionLegacy as logPolicyDecision, logPlatformAttemptLegacy as logPlatformAttempt, logPlatformResultLegacy as logPlatformResult, logRetryScheduled, logCircuitBreaker, logArtifact, logPipelineStart, logPipelineEnd, getEventFiles, checkIntegrity, } from '../ledger.js';
import { readEventsByDate, findByFingerprint, findByTraceId, findByPlatform, findByStatus, getLatestEvents, getTopDenyReasons, query, } from '../reader.js';
import { generateSummary, formatSummaryAsMarkdown, generateDailySummary, } from '../summarize.js';
// Setup/teardown
beforeAll(() => {
...
describe('MoltEvent Types', () => {
    it('should create event with all required fields', () => {
        const event = createEvent('orchestrator', 'POLICY_DECISION', 'ok', 'Test event', { test: true });
    ...
    it('should create event with optional fields', () => {
        const event = createEvent('distribution', 'PLATFORM_RESULT', 'ok', 'Posted successfully', { success: true }, {
            platform: 'x',
            action_type: 'post',
            fingerprint: 'abc123',
...
describe('Ledger Write Operations', () => {
    ...
    it('should log policy decision', () => {
        const event = logPolicyDecision('orchestrator', 'x', 'post', 'fingerprint123', true, ['ALLOWED'], 10, 'Test post about Agenium');
        expect(event.event_type).toBe('POLICY_DECISION');
        expect(event.status).toBe('ok');
        expect(event.data.allow).toBe(true);
    });
    it('should log blocked policy decision', () => {
        const event = logPolicyDecision('orchestrator', 'reddit', 'post', 'fingerprint456', false, ['BRAND_MISSING', 'QUIET_HOURS'], 10, 'Test post');
        expect(event.status).toBe('blocked');
        expect(event.data.reason_codes).toContain('BRAND_MISSING');
    });
    it('should log platform attempt', () => {
        const event = logPlatformAttempt('distribution', 'telegram', 'post', 'fp123', 'trace123', 0);
        expect(event.event_type).toBe('PLATFORM_ATTEMPT');
        expect(event.trace_id).toBe('trace123');
    });
    it('should log platform result success', () => {
        const event = logPlatformResult('distribution', 'github', 'discussion', 'fp123', 'trace123', true, {
            httpStatus: 201,
    ...
    it('should log platform result failure', () => {
        const event = logPlatformResult('distribution', 'x', 'post', 'fp123', 'trace123', false, {
            httpStatus: 429,
    ...
    it('should log retry scheduled', () => {
        const event = logRetryScheduled('distribution', 'reddit', 'post', 'fp123', 'trace123', 2, 5, 60, 'Rate limited');
    ...
    it('should log circuit breaker opened', () => {
        const event = logCircuitBreaker('distribution', 'hn', 'opened', 5, 5, new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(), 'Too many failures');
        expect(event.event_type).toBe('CIRCUIT_BREAKER_OPENED');
        expect(event.status).toBe('blocked');
    });
    it('should log circuit breaker closed', () => {
        const event = logCircuitBreaker('distribution', 'hn', 'closed', 0, 5);
        expect(event.event_type).toBe('CIRCUIT_BREAKER_CLOSED');
        expect(event.status).toBe('ok');
    });
    it('should log artifact', () => {
        const event = logArtifact('content', '/opt/marketing-ops/artifacts/draft-123.md', 'abc123def456', 'content_draft', 1024, { campaign: 'launch' });
    ...
    it('should log pipeline start and end', () => {
        ...
        const startEvent = logPipelineStart('orchestrator', runId, 'daily-content', 'schedule');
        expect(startEvent.event_type).toBe('PIPELINE_RUN_START');
        expect(startEvent.trace_id).toBe(runId);
        const endEvent = logPipelineEnd('orchestrator', runId, 'daily-content', true, 5000, 10, 8, 2);
