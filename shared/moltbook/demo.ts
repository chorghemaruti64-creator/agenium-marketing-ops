...
import { readEventsByDate, getLatestEvents, query } from './reader.js';
import { generateDailySummary } from './summarize.js';
import { MoltEvent } from './types.js';
...
const policyDecisions = [
  { platform: 'x' as const, allow: true, reasons: ['ALLOWED'], text: 'Check out Agenium v2.0!' },
  { platform: 'reddit' as const, allow: false, reasons: ['BRAND_MISSING'], text: 'Cool new protocol' },
  { platform: 'telegram' as const, allow: true, reasons: ['ALLOWED'], text: 'Agenium marketplace update' },
  { platform: 'x' as const, allow: false, reasons: ['QUIET_HOURS'], text: 'Late night Agenium post' },
  { platform: 'github' as const, allow: true, reasons: ['ALLOWED'], text: 'New discussion about agent://' },
];
...
const attempts = [
  { platform: 'x' as const, success: true, permalink: 'https://x.com/agenium/status/123' },
  { platform: 'telegram' as const, success: true, permalink: 'https://t.me/agenium/456' },
  { platform: 'github' as const, success: false, error: 'Rate limited' },
];
...
for (const att of attempts) {
  ...
  // Result
  const resultEvent = logPlatformResult(
    'distribution',
    att.platform,
    'post',
    fp,
...
const retryEvent = logRetryScheduled(
  'distribution',
  'github',
  'post',
...
const artifactEvent = logArtifact(
  'content',
  '/opt/marketing-ops/artifacts/draft-launch.md',
  'abc123def456789',
  'content_draft',
  2048,
  { campaign: 'v2-launch', version: 3 }
);
...
function printEvent(event: MoltEvent) {
